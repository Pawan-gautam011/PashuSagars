...
function Message() {
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      ...
      // If there was a doctor_id in the location state, select that doctor
      if (location.state?.doctor_id) {
        const doctorId = Number(location.state.doctor_id);
...
useEffect(() => {
  const setupWebSocket = async () => {
    try {
      ...
      // Don't show the refresh message, just indicate offline mode
      setError("Using offline mode - messages will be delivered when possible");
      setConnected(false);
    }
  };
  ...
  // Send message via WebSocket and REST API
  const sendMessage = async () => {
    ...
    // Clear message input first for better UX
          ...
          <div className="text-center mb-8">
            ...
            {!connected && (
              <div className="mt-2 text-yellow-300 text-sm">
                ⚠️ Not connected to real-time chat. Messages may be delayed.
              </div>
            )}
          </div>
                ...
                <>
                  <div className="flex items-center mb-6 pb-4 border-b border-white/10">
                    ...
                    <div className="ml-auto">
                      {connected ? (
                        <span className="inline-flex items-center bg-green-500/20 text-green-300 px-2 py-1 rounded-full text-xs">
                          <span className="w-2 h-2 bg-green-400 rounded-full mr-1"></span>
                          Connected
                        </span>
                      ) : (
                        <span className="inline-flex items-center bg-yellow-500/20 text-yellow-300 px-2 py-1 rounded-full text-xs">
                                ...
                                {message.sender === Number(user_id) && (
                                  <span>
                                    {message.is_sending ? (
                                      ...
                                      <span className="text-white/30 text-xs">Sent</span>
                                    )}
                                  </span>
                                )}
                              </div>
